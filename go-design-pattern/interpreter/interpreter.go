...
type primitiveComandNode struct {
	name string
}

func (self *primitiveComandNode) Parse(context *context) {
	self.name = context.currentToken
	context.skipToken(self.name)
}

func (self *primitiveComandNode) ToString() string {
	return self.name + " "
}
